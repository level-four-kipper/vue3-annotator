...
export class GeometricEntity {
  constructor(readonly entity: Entity, readonly ranges: Ranges, readonly level: number) {}
...
export class Ranges {
  ...
  getIntervals(startsWith: boolean, labelWidth: number): [number, number][] {
    return this.items.map((range, index) => {
      return index === 0 && startsWith ? range.getInterval(this.rtl, labelWidth) : range.getInterval()
    })
  }
}
...
export class EntityLine {
  ...
  createRanges(element: SVGTextElement, entity: Entity): Ranges {
